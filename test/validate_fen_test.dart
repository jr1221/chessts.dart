...
void main() {
  final List<({bool ok, String fen})> tests = <({bool ok, String fen})>[
    (
      // no kings
  ...
  for (final ({String fen, bool ok}) item in tests) {
    test('validateFen ${item.fen} (expected: ${item.ok})', () {
      final ({String error, bool ok}) result = Chess.validateFen(item.fen);
      if (item.ok != result.ok) {
        print(result.error);
      }
      expect(result.ok, item.ok);
    });
  }
}
