  ...
  group('Make Move - Standard Algebraic Notation (SAN)', () {
    ...
    for (final ({
      ...
      test('${position.fen} (${position.move} ${position.legal})', () {
        final Move? result =
            chess.move(san: position.move, sloppy: position.sloppy);
        if (position.legal) {
          expect(
              (result != null &&
  ...
  group('Make Move - Object', () {
    final List<
        ({
          String fen,
          bool legal,
          ({Square from, PieceSymbol? promotion, Square to}) move,
          String? next
        })> positions = <({
      String fen,
      bool legal,
      ({Square from, Square to, PieceSymbol? promotion}) move,
      String? next
    })>[
      (
        fen: 'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1',
    ...
    for (final ({
      ...
      {
        final Chess chess = Chess(fen: position.fen);
        final ({Square from, PieceSymbol? promotion, Square to}) move =
            position.move;
        test('${position.fen} (${move.to}${move.from} - ${position.legal})',
            () {
          final Move? result = chess.move(
